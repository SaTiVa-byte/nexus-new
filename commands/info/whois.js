...
module.exports = {
  ...
  category: "info",
  aliases: ["whois", "user"],
  usage: "userinfo <Erwähnen>",
  description: "Erhalte vorab Statistiken einer bestimmten Person oder sich selbst",
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      user = message.mentions.members.first() || await message.guild.members.fetch(args[0]).catch(err => { return message.channel.send(":x: Person wurde nicht gefunden") })
    }

    if (!user) {
      return message.channel.send(":x: Person wurde nicht gefunden")
    }
    ...
    if (user.user.presence.activities.length) {
      ...
      embed.addField("Gejoint am", moment(user.joinedAt).format("LLLL"))
        .addField("Account Erstellungsdatum", moment(user.user.createdAt).format("LLLL"))
      ...
      return message.channel.send(embed).catch(err => {
        return message.channel.send("Fehler : " + err)
      })
